...
# Classe: 
# - PesquisaElasticBRS : Componente python que simplifica a construção de queries no ElasticSearch
...
TESTES = ( ('dano Adj moRal','dano ADJ1 moRal'),
           ...
           ('dano com moral','dano PROX20 moral'),
...
class PesquisaElasticBRS():
    ...
    # max_query_terms=None -> maior número de termos usados na pesquisa (none é automático 80%)
    # 
    def converter_brs_elastic_more_like_this(self, criterios, campos_texto, min_term_freq=1,min_doc_freq=1, max_query_terms=25, minimum_should_match="50%" ):
    ...
    # cria os critérios do elastic baseados nos critérios do brs 
    # de forma hierárquica dos parênteses
    def converter_brs_elastic(self, criterios, campos_texto='texto'):
        if self.more_like_this:
            return self.converter_brs_elastic_more_like_this(criterios, campos_texto)
        # se não tiver parênteses e não tiver not, faz o processo mais simples
        # se não for MLT só usa o primeiro campo texto recebido (pode ser uma evolução futura)
        if type(campos_texto) is str:
           _campo = campos_texto 
        else:
            _campo = list(campos_texto)
            _campo = 'texto' if not any(_campo) else _campo[0]
        # contrói as queries e subqueries

        # grupos = [('operador', n, sublista, termos)]  >>> sublista = True/False

    def __str__(self) -> str:
        return f'PesquisaElasticBRS: {self.criterios_brs_reformatado}'
...
class PesquisaElasticBRSTeste():
    def __init__(self):
        ...
        for i, teste in enumerate(_testes):
            ...
            pbe = PesquisaElasticBRS(criterio)
            ...
            if resultado != saida:
                ...
                msg = f'TESTE PesquisaElasticBRS{_falha}: \nCritério ({_i}{_falha}):\n- Entrada: {criterio}\n- Saída:   {saida}\n- Esperado:   {resultado}\n- Lista:      {pbe.criterios_brs_listas}\n'
                raise Exception(msg)

if __name__ == "__main__":
    TESTE_DEBUG = True
    '''
    teste = ' (   (dano ADJ2 moral) "claro que  não" (moral   ADJ2 "dano material)) "e (dano prox5 material) prox teste '
    print('Teste: ', teste)
    pbe = PesquisaElasticBRS(teste)
    print('Elastic: ', pbe.criterios_brs_listas)
    print('Texto: ', pbe.criterios_brs_reformatado)

    pbe = PesquisaElasticBRS('nao inicial dano moral não (dano material) teste1 não (teste2) nao(teste3 nao teste4 (teste5 teste6)) ')
    crit, crit_not = pbe.separar_criterios_not(pbe.criterios_brs_listas)
    print('Critérios: ', crit)
    print('Critérios not: ', crit_not)
    elastic = pbe.converter_brs_elastic_more_like_this(pbe.criterios_brs_listas,"texto")
    print('Elastic MLT: ', json.dumps(elastic))
    print('--------------------------------------')
    #teste = '(dano moral e material ou casa) não (nada adj3 teste) adj5 outro prox10 fim'
    #teste = 'teste1 adj2 teste2 prox3 teste3'
    #teste = ' nao teste1 adj2 teste2 prox3 teste3 nao ( teste4 adj2 teste5)'
    #teste = 'teste0 teste1 adj2 teste2 prox3 teste3 (teste4 (teste5 ou (teste6 adj3 teste7)) teste8)'
    '''
